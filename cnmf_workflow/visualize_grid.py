...
def read_data(path_to_kikuchi, path_to_phasemap, save_path = None):
    """
    ...
    utils._plot_grid(grid, img, save_path = save_path)
...
def set_component(x_range, y_range, path, grid, image_path, plot_flag= True, save_path= None):
    x_l = x_range[0]
    x_u = x_range[1]
    ...
    if plot_flag:
        utils._plot_component(roi, grid, img, save_path=save_path)
    else:
        pass
    # print(roi)
...
def set_ROI(x_range, y_range, path, grid, path_to_phasemap, save_path = None):
    """
    ...
    img = mpimg.imread(path_to_phasemap)
    utils._plot_ROI(roi, grid, img, save_path = save_path)
